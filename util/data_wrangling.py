...
def get_train_val_test_dfs(df, train_ratio=0.6, val_ratio=0.2, test_ratio=0.2):
    n = len(df)
    n_train = int(n * train_ratio)
    n_val = int(n * val_ratio)
    n_test = n - n_train - n_val

    train_df, temp_df = train_test_split(df, train_size=n_train, random_state=42)
    val_df, test_df = train_test_split(temp_df, train_size=n_val, random_state=42)
